...
vesc::vesc(socketcan::master& can_master, uint8_t controller_id)
    : serial_(io_ctx_),
...
int vesc::get_erpm() {
  ...
  socketcan::slave_status slave_status = get_status_unchecked();
...
socketcan::slave_status vesc::get_status_unchecked() {
